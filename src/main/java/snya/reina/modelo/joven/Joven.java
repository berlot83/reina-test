...
public class Joven implements Serializable {
	...
	public void agregarSituacionTramite(JovenDAO daoJoven, TipoDeSituacionTramite tipo, Date fecha,
			TipoDeDocumento tipoDeDocumentoPosee, String numeroDocumentoPosee, Boolean actualizado,
			Boolean partidaDeNacimientoPosee, String observacionPosee, Boolean tieneDocumento,
			TipoDeDocumento tipoDeDocumentoEnTramite, String numeroDocumentoEnTramite,
			Boolean partidaDeNacimientoEnTramite, String observacionEnTramite, Boolean certificadoDeParto,
			Boolean partidaDeNacimiento, String observacionNoPosee, String observacionSeDesconoce)
		...
		// <<procesamiento>>
		SituacionTramite situacion = tipo.crearSituacion(this.getPersona(), fecha, tipoDeDocumentoPosee,
				numeroDocumentoPosee, actualizado, partidaDeNacimientoPosee, observacionPosee, tieneDocumento,
				tipoDeDocumentoEnTramite, numeroDocumentoEnTramite, partidaDeNacimientoEnTramite, observacionEnTramite,
	...
	public void modificarSituacionTramite(SituacionTramite situacion, SituacionTramite situacionNuevo)
			throws ReinaException {
		// <<validaciones>>
		...
		if (this.getSituacionesTramiteDocumento().size() > 1) {
			...
			// tiempo
			SituacionTramite situacionAnterior = this.getSituacionesTramiteDocumento()
					.get(this.getSituacionesTramiteDocumento().size() - 1);
			if (situacionAnterior.getFechaSituacion().after(situacionNuevo.getFechaSituacion())) {
				throw new ReinaException(ReinaCte.Format(ReinaCte.FECHA_SITUACION_TRAMITE_ES_MENOR,
	...
	// Familiares
	//
	public Familiar agregarFamiliar(Persona persona, Boolean vive, Boolean convive, Boolean referente, Boolean tutor,
			String observacion, ObraSocial obraSocial, TipoDeParentesco parentesco) throws ReinaException {
		ArbolFamiliar familia = new ArbolFamiliar();
		ResultadoFamiliar resultado;

		resultado = familia.generarFamiliar(this, persona, vive, convive, referente, tutor, observacion, obraSocial,
				parentesco);
	...
	public void modificarFamiliar(Familiar familiar, Boolean tieneDocumento, TipoDeDocumento tipoDeDocumento,
			String numeroDocumento, Boolean vive, Boolean convive, Boolean referente, Boolean tutor, String observacion,
			ObraSocial obraSocial, TipoDeParentesco parentesco) throws ReinaException {
		ArbolFamiliar familia = new ArbolFamiliar();
		ResultadoFamiliar resultado;

		resultado = familia.modificarFamiliar(this, familiar, tieneDocumento, tipoDeDocumento, numeroDocumento, vive,
				convive, referente, tutor, observacion, obraSocial, parentesco);
	...
	// Modifica el domicilio del familiar que convive con el joven para que tenga el
	// mismo domicilio
	public void modificarDomicilioFamiliarConviviente(Familiar familiar) {
		if (this.getDomicilio() != null)
			familiar.setDomicilio(obtenerDomicilioJoven());
	}
	...
	public String getDetalleReferentes() {
		if (this.getFamiliares() != null) {
			Iterator<Familiar> iter = familiares.iterator();
			String detalle = "";
			while (iter.hasNext()) {
				Familiar familiar = iter.next();
				/* Se agregó este condicional AAB */
				if(familiar.getNombreCompleto() != null || familiar.getParentesco().getNombre() != null) {
	...
	private Domicilio obtenerDomicilioJoven() {
		...
		String domicilioCalle, domicilioNumero, domicilioPiso, domicilioTorre, domicilioDepto, domicilioEntre1,
				domicilioEntre2, referenciaDomicilio;
		...
		if (this.getDomicilio().getLocalidad() != null)
			localidadDomicilio = this.getDomicilio().getLocalidad();
		else
			localidadDomicilio = null;

		// Si tiene Barrio lo guarda
		if (this.getDomicilio().getBarrio() == null)
			barrioDomicilio = null;
		else
			barrioDomicilio = this.getDomicilio().getBarrio();

		if ((this.getDomicilio().getTieneReferenciaDomiciliaria() != null)
				&& (this.getDomicilio().getTieneReferenciaDomiciliaria() != false)) {
		...
		return this.crearDomicilio(domicilioExacto, domicilioCalle, domicilioNumero, domicilioPiso, domicilioTorre,
				domicilioDepto, domicilioEntre1, domicilioEntre2, provinciaDomicilio, municipioDomicilio,
				localidadDomicilio, barrioDomicilio, referenciaDomicilio);

	}

	// Carga los datos del domicilio a una variable domicilio
	private Domicilio crearDomicilio(Boolean domicilioExacto, String domicilioCalle, String domicilioNumero,
			String domicilioPiso, String domicilioTorre, String domicilioDepto, String domicilioEntre1,
			String domicilioEntre2, Provincia provincia, Municipio municipio, Localidad localidad, Barrio barrio,
	...
	public CorteDeProcesoPenal agregarProcesoPenal(AdministradorDeProceso dao, Date fechaIngreso,
			OrganoJudicial juzgado, Fiscalia fiscalia, Boolean esDefensorOficial, Defensoria defensoria,
			ContactoInstitucion defensor, String abogado, String iPP, String nroCarpeta, String nroCausa,
			TipoDeMotivoIntervencion tipoDeMotivo, Boolean gradoTentativa, String observacionCaratula,
			TipoDeMomentoProcesal tipoMomentoProcesal, TipoDeMedidaEnProcesoPenal tipoMedida,
			List<TipoDeDetalleDeMedidaEnProcesoPenal> detallesMedida, String observacionMedida, Integer horasDeMedida,
			Integer diasDeMedida, Integer mesesDeMedida, Date fechaDeMedida) throws ReinaException {
		ProcesoPenal proceso = new ProcesoPenal(fechaIngreso, juzgado, fiscalia, esDefensorOficial, defensoria,
				defensor, abogado, iPP, nroCarpeta, nroCausa, this);

		proceso.agregarMotivoIntervencion(juzgado, fechaIngreso, tipoDeMotivo, gradoTentativa, observacionCaratula);
		proceso.agregarMomentoProcesal(dao, tipoMomentoProcesal, fechaIngreso, juzgado, fiscalia, tipoMedida,
				detallesMedida, observacionMedida, horasDeMedida, diasDeMedida, mesesDeMedida, fechaDeMedida);

		this.getProcesos().add(proceso);
		proceso.setJoven(this);

		return new CorteDeProcesoPenal(proceso, proceso.getMotivoIntervencion(), proceso.getMomentoProcesal(),
				proceso.getMedidaImpuesta(), proceso.getSituacionProcesal());
	}
	...
	// Intervenciones
	//
	public IntervencionPenal agregarIntervencionPenal(Intervencion intervencion, Date fechaIntervencion,
			Recurso recurso, String observacion, MedidaEnProcesoPenal medidaImpuesta,
			EscritorNarrativoDeHistoria escritor) {

		CentroDeReferencia centroDeReferencia = (CentroDeReferencia) recurso.getComponente();
		IntervencionPenal intervencionP = new IntervencionPenal(intervencion, fechaIntervencion, centroDeReferencia,
				medidaImpuesta, observacion);
	...
	public void cesarIntervencionPenal(IntervencionPenal intervencion, Date fechaBaja,
			MotivoBajaIntervencion motivoDeBaja, String observacionCese, EscritorNarrativoDeHistoria escritor)
			throws ReinaException {
	...
	//
	public Informe agregarInforme(Intervencion intervencion, Date fecha, TipoDeInforme tipo, String observacion,
			Institucion institucion, String autores, Boolean conArchivo, TipoDeEstadoInformeEnum estado)
			throws ReinaException {
	...
	public Movimiento traerUltimoMovimiento() {
		return (this.getMovimientos().size() > 0) ? this.getMovimientos().get(this.getMovimientos().size() - 1) : null;
	}
	...
	public void recibirLegajo(Date fecha, Institucion institucion, String observacion) {
		...
		String obs = "El dia " + Calendario.formatearFecha(fecha) + " se recibe el legajo de la institucion "
				+ institucion.getNombre();
		if (observacion != null && !observacion.equals("")) {
			obs = obs + ", donde " + observacion;
		}
		obs = ((this.getLegajo().getObservacion() == null || this.getLegajo().getObservacion().equals("")) ? ""
				: (this.getLegajo().getObservacion() + ". ")) + obs;
	...
	public void enviarLegajo(Date fecha, Institucion institucion, String observacion) {
		...
		String obs = "El dia " + Calendario.formatearFecha(fecha) + " se entrega el legajo a la institucion "
				+ institucion.getNombre();
		if (observacion != null && !observacion.equals("")) {
			obs = obs + ", donde " + observacion;
		}
	...
	// Historial Presente
	//
	public void agregarPresenteEnRecurso(Intervencion intervencion, Movimiento movimiento, ProcesoPenal proceso,
			AmbitoEjecucion ambitoEjecucionOrigen, AmbitoEjecucion ambitoEjecucionDestino,
			EscritorNarrativoDeHistoria escritor) {
		ResultadoPresente resultado;

		resultado = movimiento.asentarHistorial(this, intervencion, proceso, ambitoEjecucionOrigen,
				ambitoEjecucionDestino, escritor);
		resultado.inscribirEnJoven(this);
	}

	public void cesarPresenteEnRecurso(IntervencionPenal intervencion, EscritorNarrativoDeHistoria escritor)
			throws ReinaException {
		...
		if (presente == null)
			throw new ReinaException("El joven no se encuentra actualmente vinculado al centro de referencia "
	...
	public void cambiarCondicionDePermanenciaPorUnificacion(Date fecha, ProcesoPenal proceso,
			EscritorNarrativoDeHistoria escritor) {
		List<Permanencia> complementos = new ArrayList<Permanencia>();

		for (Permanencia permanencia : this.getPermanencias()) {
			if (permanencia.estaVigente() && permanencia.getProceso() != null
					&& permanencia.getProceso().getId().equals(proceso.getId()))
				complementos.add(
						permanencia.armarContinuidadPorUnificacion(fecha, proceso.getProcesoUnificador(), escritor));
		}
	...
	public void retrotraerCondicionDePermanenciaPorUnificacion(Date fecha, ProcesoPenal proceso,
			EscritorNarrativoDeHistoria escritor) {
		HashMap<Integer, Permanencia> complementos = new HashMap<Integer, Permanencia>();

		for (Permanencia permanencia : this.getPermanencias()) {
			if (permanencia.getProceso() != null && permanencia.getProceso().getId().equals(proceso.getId())
					&& permanencia.getFechaFin().equals(fecha)) {
	...
	public MedidaEnProcesoPenal traerUltimaMedida() {
		ArrayList<ProcesoPenal> procesos = new ArrayList<ProcesoPenal>(this.getProcesos().size());
		...
		Comparator<ProcesoPenal> proceso_orden = new Comparator<ProcesoPenal>() {
			public int compare(ProcesoPenal p1, ProcesoPenal p2) {
				return p1.getMedidaImpuesta().getFechaMedida().compareTo(p2.getMedidaImpuesta().getFechaMedida());
			}
		};
	...
	public String getDetalleLugarDeNacimiento() {
		...
		if (this.getLocalidad() != null)
			descripcion = "Localidad de " + this.getLocalidad().getNombre();
		if (this.getMunicipio() != null)
			descripcion += ((descripcion.length() > 0) ? ", " : "") + "Municipio de " + this.getMunicipio().getNombre();
		if (this.getProvincia() != null)
			descripcion += ((descripcion.length() > 0) ? ", " : "") + "Provincia de " + this.getProvincia().getNombre();
	...
	public String getDetalleHistoricidad() {
		boolean tieneIntervCAD = false;
		boolean estaIntervCAD = false;
		boolean tieneIntervContextoEncierro = false;
		boolean estaIntervContextoEncierro = false;
		boolean tieneIntervMonitoreo = false;
		boolean estaIntervMonitoreo = false;

		// for (Intervencion intervencion : this.getIntervenciones()) {
		// if
		// (intervencion.getTipo().getId().equals(TipoDeIntervencion.ID_ADMISION_CAD)) {
		// tieneIntervCAD = true;
		// estaIntervCAD = estaIntervCAD || ( intervencion.getVigente() );
		// }
		// if
		// (intervencion.getTipo().getId().equals(TipoDeIntervencion.ID_CONTEXTO_ENCIERRO))
		// {
		// tieneIntervContextoEncierro = true;
		// estaIntervContextoEncierro = estaIntervContextoEncierro || (
		// intervencion.getVigente() );
		// }
		// if
		// (intervencion.getTipo().getId().equals(TipoDeIntervencion.ID_MONITOREO_TERRITORIAL))
		// {
		// tieneIntervMonitoreo = true;
		// estaIntervMonitoreo = estaIntervContextoEncierro || (
		// intervencion.getVigente() );
		// }
		// }
	...
	private void compruebaPoderAgregarSituacionDocumento(TipoDeSituacionTramite tipo, Date fecha)
			throws ReinaException {
		if (this.getSituacionesTramiteDocumento().size() > 0) {
			SituacionTramite ultima = this.getUltimaSituacionesTramiteDocumento();
			...
			if (!ultima.getTipo().getPosibles().contains(tipo)) {
				throw new ReinaException(ReinaCte.Format(ReinaCte.NO_POSIBLE_SITUACION_TRAMITE_SIGUIENTE,
						ultima.getTipo().getNombre(), ultima.getTipo().getNombre()));
			}
			// valida que el dato a ingresar sea mayor que el acutal segun el tiempo
			if (ultima.getFechaSituacion().compareTo(fecha) > 0) {
				throw new ReinaException(ReinaCte.Format(ReinaCte.FECHA_SITUACION_TRAMITE_ES_MENOR,
						ultima.getFechaSituacion().toString()));
			}
		}
	}
	...
	public void agregarBeneficio(TipoDeBeneficio tipo, String numero, Date fechaAltaBeneficio,
			String observacionBeneficio, EstadoBeneficio estadoBeneficio, Date fechaEntregaTarjeta) {
		// TODO
		BeneficioDelJoven beneficio = new BeneficioDelJoven(tipo, numero, fechaAltaBeneficio, estadoBeneficio,
				observacionBeneficio, null);
	...
	public void agregarObraSocial(ObraSocial obraSocial, String numeroCarnet, Date fVencimientoCarnet,
			String observacionObraSocial, EstadoObraSocial estado) {
		if (this.getObraSocial() == null) {
			this.setObraSocial(new CoberturaObraSocial(obraSocial, numeroCarnet, fVencimientoCarnet,
					observacionObraSocial, estado));
		} else {
	...
	public void agregarDiscapacidad(TipoDeDiscapacidad tipoDeDiscapacidad, Integer porcentajeDiscapacidad,
			Boolean certificadoDiscapacidad, Date fDiscapacidad, String observacionDiscapacidad,
			EstadoObraSocial estadoSeDesconoce) {
		if (this.getObraSocial() == null) {
			this.setObraSocial(new CoberturaObraSocial(tipoDeDiscapacidad, porcentajeDiscapacidad,
					certificadoDiscapacidad, fDiscapacidad, observacionDiscapacidad));
	...
	public boolean tieneDatosDeCoberturaSocialOPension() {
		...
		while (!tienePension && it.hasNext()) {
			...
			if (ben.getTipo().getGrupo().getId().equals(GrupoDeBeneficio.ID_BENEFICIO_PENSION)
					&& ben.getFechaBaja() != null)
				tienePension = true;
		}
	...
	public void setSituacionesTramiteDocumento(java.util.List<SituacionTramite> value) {
		situacionesTramiteDocumento = value;
	}
	
	@NotFound(action = NotFoundAction.IGNORE) /* Agregado AAB */
	...
	/*
	 * public java.util.List<InscripcionPrograma> getProgramas() { return programas;
	 * }
	 * 
